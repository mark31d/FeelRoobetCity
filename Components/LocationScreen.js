...
const COLORS = {
  ...
  // фиолетовые тона как в Onboarding
  cardBg: 'rgba(58, 20, 117, 0.35)',
  infoBg: 'rgba(58, 20, 117, 0.55)',
  infoBg2: 'rgba(23, 10, 46, 0.60)',
...
function getMapsEmbedUrl(address) {
  if (!address || address.trim() === '') {
    return 'https://www.google.com/maps/@46.0101,8.9600,13z';
  }
  // Используем обычный URL карты Google Maps для поиска
  const encodedAddr = encode(address);
  return `https://www.google.com/maps/search/?api=1&query=${encodedAddr}`;
}

function getMapsEmbedHtml(address) {
  if (!address || address.trim() === '') {
    return '<html><body style="margin:0;padding:0;"><iframe width="100%" height="100%" frameborder="0" style="border:0" src="https://www.google.com/maps/@46.0101,8.9600,13z" allowfullscreen></iframe></body></html>';
  }
  const encodedAddr = encode(address);
  return `<html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"></head><body style="margin:0;padding:0;"><iframe width="100%" height="100%" frameborder="0" style="border:0" src="https://www.google.com/maps?q=${encodedAddr}&output=embed&z=15" allowfullscreen></iframe></body></html>`;
}
          ...
          {showMap && address && (
            <View style={styles.mapOuter}>
              <View style={styles.mapClip}>
                <WebView
                  source={{ html: getMapsEmbedHtml(address) }}
                  style={styles.web}
                  javaScriptEnabled
                  domStorageEnabled
                  originWhitelist={['*']}
                  ...
                  renderError={(errorName) => (
                    <View style={styles.mapError}>
                      ...
                      <TouchableOpacity onPress={onOpenMaps} style={styles.mapErrorBtn}>
                        <Text style={styles.mapErrorBtnText}>Open in Maps App</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                />
