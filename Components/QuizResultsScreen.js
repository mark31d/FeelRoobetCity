    ...
    <View style={styles.root}>
      ...
      >
        ...
        <View style={[styles.resultTop, { paddingTop: insets.top + 10, paddingHorizontal: 16 }]}>
          <CircleBack
            onPress={() => {
              // Возвращаемся назад: сначала пробуем goBack в текущем стеке, если не получается - выходим из QuizStack
              if (navigation.canGoBack()) {
                navigation.goBack();
              } else {
                // Если не можем вернуться назад в стеке, выходим из QuizStack
...
const styles = StyleSheet.create({
  ...
  resultTop: { flexDirection: 'row', alignItems: 'center', zIndex: 10 },
  resultHero: { flex: 1, justifyContent: 'flex-end', alignItems: 'center' },
